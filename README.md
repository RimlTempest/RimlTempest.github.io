...
- **キャラクターの図版は 1 枚のデザインシートから切り出しています。** この絵は線画を
  持たない平坦な塗りで、紙 `rgb(252,253,253)` と白い上着 `rgb(253,253,253)` の差は
  **1** しかありません。色を見るどんな方法でも分けられないので、アニメ絵向けの
  学習済みモデル（rembg の `isnet-anime`）が形で判断したマスクを土台にし、
  モデルが落とす細いアホ毛だけを色で補っています（意味マスクと繋がっている
  かたまりに限る）。境界のアルファは `観測色 = a×前景 + (1-a)×背景` から解き直すので、
  白フチが原理的に出ません。検品はマゼンタ地に置いて拡大して行います。
